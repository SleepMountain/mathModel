...
for f in file:
    ...
    with open("Task1.1-" + f + "-" + "result.csv", newline='', encoding='gbk') as csvfile:
        reader = list(csv.reader(csvfile))
        for rowid in range(1, len(reader)):
            data = reader[rowid]
            result[f][data[0]][data[1]]["total_create"] = int(data[2])
            result[f][data[0]][data[1]]["success_create"] = int(data[3])
            result[f][data[0]][data[1]]["error_create"] = int(data[4])
            result[f][data[0]][data[1]]["error_rate"] = float(data[5])

resultcsvwitheachmonth = []
for f in file:
    for month in result[f]:
        for date in result[f][month]:
            resultcsvwitheachmonth.append([
                month + "-" + date,
                ...
                result["M101"][month][date]["total_create"],
                result["M101"][month][date]["success_create"],
                result["M101"][month][date]["error_create"],
                result["M101"][month][date]["error_rate"],
                result["M102"][month][date]["total_create"],
                result["M102"][month][date]["success_create"],
                result["M102"][month][date]["error_create"],
                result["M102"][month][date]["error_rate"]
            ])
with open("Task1.t-" + "result.csv", 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(
        ["时间", "M101有效工作时长", "M102有效工作时长", "M101日产量", "M101日成功产量", "M101日失败产量", "M101成功率",
         "M102日产量", "M102日成功产量", "M102日失败产量", "M102成功率"])
    for row in resultcsvwitheachmonth:
        writer.writerow(row)
