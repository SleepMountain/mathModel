...
# ==========================================因子检测参数设置==============================================================
    ...
    factor_dp = {
        "vov_factor": {"fillna": 1, "winsorize": 2, "standardize": 2},
    }
    ...
    def sfactor_data(self, start_date, end_date, stocks, names):
        ...
        # 计算日波动率
        daily_volatility = log_returns.pow(2)
        
        # 计算VOV因子（在每个时间点上计算）
        vov = np.sqrt(daily_volatility.rolling(window=10).std() / daily_volatility.rolling(window=10).mean())
        
        
        # 返回结果
        return {'vov_factor': vov.T}
