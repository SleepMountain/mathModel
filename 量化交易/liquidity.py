...
# ==========================================因子检测参数设置==============================================================
    ...
    factor_dp = {
        "liquidity": {"fillna": 1, "winsorize": 2, "standardize": 2},
    }
    ...
    def sfactor_data(self, start_date, end_date, stocks, names):
        ...
        turnover_rate = get_price(stocks, start_date, end_date, "1d", ["close","turnover_rate"], bar_count=0, skip_paused=False, fq="pre", is_panel=1)["turnover_rate"]
        log.info(turnover_rate)
        # 交易天数
        trading_days = {'monthly': 21, 'quarterly': 63, 'yearly': 252}
        
        # 权重
        weights = {'monthly': 0.35, 'quarterly': 0.35, 'yearly': 0.3}
        
        # 计算各周期换手率
        turnover_rates = {}
        for period, days in trading_days.items():
            turnover_rates[period] = turnover_rate.rolling(window=days).sum().apply(np.log)
        
        # 将所有换手率 DataFrame 合并
        all_turnovers = pd.concat(turnover_rates.values(), 
                          keys=trading_days.keys(),
                          axis=1,
                          names=['period'])
        
        # 计算 Liquidity 因子
        liquidity_factor = (
            weights['monthly'] * all_turnovers.xs('monthly', level=0, axis=1) +
            weights['quarterly'] * all_turnovers.xs('quarterly', level=0, axis=1) +
            weights['yearly'] * all_turnovers.xs('yearly', level=0, axis=1)
        )
        
        # 返回结果
        return  {'liquidity': liquidity_factor.T}
