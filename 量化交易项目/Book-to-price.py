...
# ==========================================因子检测参数设置==============================================================
    ...
    factor_dp = {
        "Book-to-price": {"fillna": 1, "winsorize": 2, "standardize": 2},
    }
    ...
    factor_set = {
        "Book-to-price": {"direct": 1, "weight": 1},
    }
    ...
    def sfactor_data(self, start_date, end_date, stocks, names):
        log.info("获取市净率")
        pb_value = get_sfactor_data(start_date, end_date, stocks, ["pb"])["pb"]
        # 计算 Book-to-Price 比率
        bp_factor = 1 / pb_value
        log.info(bp_factor)

        # 确保返回结果的格式正确
        bp_factor = bp_factor.T  # 转置 DataFrame，使得列名为时间，行名为股票代码
        bp_factor.columns = bp_factor.columns.astype(str)  # 确保列名是字符串类型

        return {"Book-to-price": bp_factor.T}
